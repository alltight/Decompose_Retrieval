...
class HNSW(object):
    ...
    def __init__(self, distance_type, m=5, ef=200, m0=None, heuristic=True, vectorized=False, distance_func=None):
        self.data = []
        if distance_type == "l2":
            # l2 distance
    ...
    def search(self, q, k=None, ef=None, distance=None):
        ...
        for layer in reversed(graphs[1:]):
            point, dist = self._search_graph_ef1(q, point, dist, layer, distance)
        # look for ef neighbors in the bottom level
        ep = self._search_graph(q, [(-dist, point)], graphs[0], ef, distance)
    ...
    def _search_graph_ef1(self, q, entry, dist, layer, distance=None):
        ...
        def vectorized_distance(x, ys):
            return [distance(x, y) for y in ys]
    ...
    def _search_graph(self, q, ep, layer, ef, distance=None):
